<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:85:"E:\phpStudy\PHPTutorial\WWW\myAdmin\public/../application/admin\view\login\index.html";i:1554621367;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8"/>
    ...
    function check() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 1) {
                    layer.msg('登录成功', {icon: 6, time: 900});
                    setTimeout("location.href='/admin/index'");

                }
                if (data == 2) {
                    layer.msg('密码错误', {icon: 5, time: 900});

                }
                if (data == 3) {
                    layer.msg('管理员错误', {icon: 5, time: 900});

                }
                if (data == 4) {
                    layer.msg('验证码错误', {icon: 5, time: 900});
                }
            },
            error: function (e) {
                alert(e);
